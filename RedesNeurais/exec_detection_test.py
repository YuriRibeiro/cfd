...
$ python exec_detection_test.py -e YOLOv5_UAVDT_0
"""
...
yv5_weigth_path = lambda exp, date: os.path.join(os.getcwd(),exp,
...
class main():
    def __init__(self, opt):
        ...
        if self.experimento not in weight_paths.keys():
            raise Exception(f"Experimento '{self.experimento}' não encontrado." +\
                             f"Experimentos disponíveis: {weight_paths.keys()}.")
        self.run_inference()     
    ...
    def run_inference(self):
        ...
        network = self.experimento.split("_")[0:2]
        network = "_".join(network)

        if network == "YOLOv5_UAVDT":

            self.project = "YOLOv5_UAVDT_det"
            self.name = weight_paths[self.experimento].split(os.sep)[-3]
            ...
            yv5_path = os.path.join(os.getcwd(), self.experimento, f"yolov5_w_det_temp")
            ...
            self.output_dir_path = os.path.join(os.getcwd(), self.experimento, self.project, self.name)
            ...
            data_yaml_path = os.path.join(os.getcwd(), self.experimento, "data_detection.yaml")
            ...
            test_set_det = { k:[] for k in movies_teste}
            with open(json_path) as f:
                data = json.load(f)

            for ann in data:
                vid, img_id = ann['image_id'].split("_")
                if not vid in movies_teste: continue
                x,y,w,h = ann['bbox']
                x, w = float(x), float(w)
                y, h = float(y), float(h)
                score = ann['score']

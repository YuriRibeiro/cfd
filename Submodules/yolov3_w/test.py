...
def test(data,
    ...
    if training:  # called by train.py
        ...
        # Directories
        save_dir = Path(increment_path(Path(opt.project) / opt.name / f"{imgsz}x{imgsz}", exist_ok=opt.exist_ok))  # increment run
        (save_dir / 'labels' if save_txt else save_dir).mkdir(parents=True, exist_ok=True)  # make dir

        # Load model
        model = attempt_load(weights, map_location=device, allow_download = False)  # load FP32 model
    ...
    if not training:
        ...
        with open( str(save_dir / "detection_speed.txt"), 'w') as f:
